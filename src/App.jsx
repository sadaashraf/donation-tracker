...
import PaymentsBottomNav from "./navigation/PaymentsBottomNav";
  ...
  return (
    <aside className="hidden md:flex flex-col w-20 xl:w-56 bg-white border-r border-gray-100 flex-shrink-0">
      <div className="flex items-center gap-3 px-4 py-5 border-b border-gray-100">
        <AppLogo />
        <span className="hidden xl:block text-base font-bold text-gray-900 truncate">MemberFlow</span>
      </div>
      ...
      <div className="p-3 border-t border-gray-100">
        <div className="flex items-center gap-3 px-3 py-2">
          <div className="w-8 h-8 rounded-full bg-blue-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">AJ</div>
...
export default function App() {
  ...
  const navigateTo = (s) => { setScreen(s); setSubScreen(null); setModal(false); };
  ...
  const renderContent = () => {
    ...
    if (screen === "members")   return <MembersScreen setModal={setModal} />;
  ...
  const isPayments  = screen === "payments";
  ...
  return (
    <div className="min-h-screen bg-gray-50 md:bg-gray-200 flex flex-col md:flex-row">
      ...
      {/* Main content — full screen on mobile, flex-1 on tablet+ */}
      <main className="flex-1 flex flex-col overflow-hidden relative min-h-screen md:min-h-0">
        ...
        {!isSubScreen && !isPayments && (
          <div className="md:hidden">
            <BottomNav active={screen} setScreen={navigateTo} />
          </div>
        )}
        {isPayments && (
          <div className="md:hidden">
            <PaymentsBottomNav setScreen={navigateTo} />
          </div>
        )}
      </main>
    </div>
